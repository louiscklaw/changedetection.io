$(document).ready(function () {
  ...
  setInterval(() => {
    if (browserless_seconds_remaining >= 1) {
      document.getElementById('browserless-seconds-remaining').innerText =
        browserless_seconds_remaining + ' seconds remaining in session';
  ...
  function set_scale() {
    ...
    $('#browsersteps-selector-canvas')
      .attr('height', selector_image_rect.height)
      .attr('width', selector_image_rect.width);
    ...
    $('#browsersteps-selector-canvas').bind(
      ...
      function (e) {
        ...
        xpath_data['size_pos'].forEach(function (item, index) {
          ...
          if (
            e.offsetY > item.top * y_scale &&
            e.offsetY < item.top * y_scale + item.height * y_scale &&
            e.offsetX > item.left * y_scale &&
            e.offsetX < item.left * y_scale + item.width * y_scale
          ) {
        ...
        if (possible_elements.length) {
          ...
          ctx.strokeRect(
            current_selected_i.left * x_scale,
            current_selected_i.top * y_scale,
            current_selected_i.width * x_scale,
            current_selected_i.height * y_scale,
          );
          ctx.fillRect(
            current_selected_i.left * x_scale,
            current_selected_i.top * y_scale,
  ...
  $('ul#browser_steps li').each(function (i) {
    var s =
      ...
      '<a data-step-index=' +
      i +
      ' class="pure-button button-secondary button-green button-xsmall apply" >Apply</a>&nbsp;';
    if (i > 0) {
      // The first step never gets these (Goto-site)
      s +=
        '<a data-step-index=' +
        i +
        ' class="pure-button button-secondary button-xsmall clear" >Clear</a>&nbsp;' +
        '<a data-step-index=' +
        i +
        ' class="pure-button button-secondary button-red button-xsmall remove" >Remove</a>';
    }
    s += '</div>';
    $(this).append(s);
  });
